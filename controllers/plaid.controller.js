...
const getTransactions = async function (request, response, next) {
  ...
  try {
    ...
    transactions.map((t) => {
      ...
      gwei = new BigNumber(String(parseInt(sat)))
        .plus(new BigNumber(gwei))
        .toString();
      ...
      if (t.merchant_name !== null) {
        recent.length < 1
          ...
          : recent.length > 0 &&
            time > parseInt(recent[0].createdAt) &&
            rows.push({
              measurement: 'transactions',
              tags: {
                name: t.name.replace(/[^a-zA-Z- ]/g, ''),
    ...
    try {
      await influxClient.writePoints(rows).catch((err) => {
    ...
    if (err) {
      ...
      if (user) {
        ...
        return ReS(
          response,
          { message: 'Points added.', success: true, user: user },
          200,
        );
      } else {
        ...
        return ReE(
          response,
          { message: 'Could not find the user.', success: false },
          400,
        );
      }
    }
...
const getUsers = async function (req, res) {
  ...
  if (err) {
    ...
    if (users.length > 0) {
      return ReS(
        res,
        { message: 'Users are:', success: true, users: users },
        200,
      );
    } else {
...
const createUser = async function (req, res) {
  ...
  if (typeof reqUser.ethAddress === 'undefined' || reqUser.ethAddress === '') {
    return ReE(
      res,
      { message: 'Please enter ethAddress', success: false, user: '' },
      400,
    );
  }
  ...
  if (typeof reqUser.fullName === 'undefined' || reqUser.fullName === '') {
    return ReE(
      res,
      { message: 'Please enter nickname', success: false, user: '' },
      400,
    );
  } else {
    ...
    if (err) {
      ...
      if (user != null) {
        ...
        return ReE(
          ...
          {
            message:
              'Sorry unable to create this User request. Try after sometime.',
          },
          400,
        );
      }
...
const getUser = async function (req, res) {
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'This user information is', user: user, success: true },
      200,
    );
  }
};
...
const checkUserName = async function (req, res) {
  ...
  [err, user] = await to(
    Users.find({ fullName: new RegExp(`^${req.params.name}`, 'i') }),
  );
  if (err) {
    return ReE(res, err, 400);
  } else {
...
const updateUser = async function (req, res) {
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'user updated', user: user, success: true },
      200,
    );
  }
};
...
const deleteUser = async function (req, res) {
  ...
  id = req.query.id;
  body = req.body[(err, user)] = await to(
    Users.findByIdAndUpdate(id, { $set: { active: false } }, { new: true }),
  );

  if (err) {
    return ReE(
      res,
      { message: 'Error on retrieving user', err, success: false },
      400,
    );
  } else {
    return ReS(
      res,
      { message: 'User deleted', user: user, success: true },
      200,
    );
  }
};
  ...
  try {
    ...
    await influxClient
      ...
      .then((result) => {
        return ReS(
          response,
          { message: 'Expenses are', success: true, transactions: result },
          200,
        );
      })
      .catch((err) => {
...
const addCategory = async function (req, res) {
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'Category is created.', success: true, category: category },
      200,
    );
  }
};
...
const deActivateCategory = async function (req, res) {
  ...
  [err, category] = await to(
    TransactionCategories.findByIdAndUpdate(
      id,
      { $set: { active: false } },
      { new: true },
    ),
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'Category removed successfully', success: true },
      200,
    );
  }
};
...
const getTransactionsByCategory = async function (req, res) {
  ...
  query = `SELECT * FROM transactions 
  where address = ${Influx.escape.stringLit(
    address,
  )} and category = ${Influx.escape.stringLit(categoryName)}`;
  ...
  try {
    await influxClient.query(query).then(async (result) => {
      ...
      [err, category] = await to(
        TransactionCategories.findOne({ name: categoryName }),
      );
      if (err) {
        return ReE(res, { err }, 400);
      }
      ...
      return ReS(
        res,
        {
          message: `Transactions of ${categoryName} category is..`,
          success: true,
          transactions: result,
        },
        200,
...
const mySpendings = async function (req, res) {
  ...
  startDate =
    req.query.startDate !== undefined && req.query.startDate !== null
      ? moment(req.query.startDate).valueOf() * 1000
      : null;

  endDate =
    req.query.endDate !== undefined && req.query.endDate !== null
      ? moment(req.query.endDate).valueOf() * 1000
      : null;

  totalQuery =
    startDate !== null && endDate !== null
      ? `SELECT sum("sats") AS "amount",count("merchant_name") as "total_transactions" FROM transactions where address = ${Influx.escape.stringLit(
          address,
        )} and time >= ${startDate} and time <= ${endDate} GROUP BY category`
      : `SELECT sum("sats") AS "amount",count("merchant_name") as "total_transactions" FROM transactions where address = ${Influx.escape.stringLit(
          address,
        )}`;
      ...
      .then(async (result) => {
        ...
        if (result.length > 0) {
            ...
            await influxClient.query(categoryWiseQuery).then(async (result) => {
              ...
              if (categories && categories.length > 0) {
                    ...
                    if (category !== null && category !== {}) {
                      if (count === spendings.length) {
                        spendings.push({
                          ...
                          percentage:
                            (parseFloat(data.total_transactions) /
                              parseFloat(total_transactions)) *
                            100,
                          amount: data.amount,
                        });
                      ...
                      }[(err, newCategory)] = await to(
                        TransactionCategories.create(body),
                      );
                ...
                return ReE(
                  ...
                  {
                    message:
                      'Transactions not found for this category.Please connect your bank and try again.',
                    success: false,
                  },
                  400,
                );
          ...
          return ReE(
            ...
            {
              message:
                'Transactions not found for this user.Please connect your bank and try again.',
              success: false,
            },
            400,
          );
