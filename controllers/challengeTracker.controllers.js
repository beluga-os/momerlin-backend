...
const trackChallenger = async function (req, res) {
  ...
  if (
    typeof body.challenge === 'undefined' ||
    body.challenge === '' ||
    body.challenge === null
  ) {
    return ReE(
      res,
      { message: 'Please provide a valid challenge', success: false },
      400,
    );
  } else if (
    typeof body.competitor === 'undefined' ||
    body.competitor === '' ||
    body.competitor === null
  ) {
    return ReE(
      res,
      { message: 'Please provide a valid challenger', success: false },
      400,
    );
  } else if (
    ...
    if (err) {
      return ReE(
        res,
        {
          message: 'Error on checking challenger is a valid user',
          success: false,
          err,
        },
        400,
      ...
      if (user) {
        ...
        if (err) {
          ...
          if (challenger.length > 0) {
            if (challenger[0].challenge) {
              body.streakNo =
                challenger[0].challenge.totalKm <= distance
                  ? challenger[0].streakNo + 1
                  : 0;

              if (
                parseInt(challenger[0].challenge.streakDays) >=
                  parseInt(body.streakNo) &&
                challenger[0].status !== 'completed'
              ) {
                body.status =
                  parseInt(challenger[0].challenge.streakDays) ===
                  parseInt(body.streakNo)
                ...
                [err, challengeTracker] = await to(
                  ChallengeTracker.findOneAndUpdate(
                    { competitor: body.competitor, challenge: body.challenge },
                    body,
                  ),
                );
                ...
                if (err) {
                  return ReE(
                    res,
                    {
                      message: 'Error on updating challenger',
                      success: false,
                      err,
                    },
                    400,
                  ...
                  return ReS(
                    res,
                    {
                      message:
                        parseInt(challenger[0].challenge.streakDays) ===
                        parseInt(body.streakNo)
                          ? 'Challenge Completed Thank you for participating.'
                          : 'Challenger record updated successfully',
              ...
              return ReS(
                res,
                { message: 'Invalid Challenge.', success: false, err },
                400,
              );
            }
          } else {
            ...
            if (err) {
              return ReE(
                res,
                { message: 'Error on checking challenge', success: false, err },
                400,
              );
            } else {
              ...
              if (challenge) {
                ...
                body.streakNo =
                  challenge.totalkm <= distance
                    ? 1
                    : ((0)[(err, challengeTracker)] = await to(
                        ChallengeTracker.create(body),
                      ));
                ...
                if (err) {
                  return ReE(
                    res,
                    {
                      message: 'Error on tracking challenger',
                      success: false,
                      err,
                    },
                    400,
                  ...
                  if (challengeTracker !== null || challengeTracker !== {}) {
                    ...
                    return ReE(
                      ...
                      {
                        message:
                          'Unable to retrieve challenges please try again.',
                        success: false,
                      },
                      400,
                    );
                ...
                return ReE(
                  res,
                  { message: 'Challenge not found', success: false, err },
                  400,
                );
              }
            }
        ...
        return ReE(
          res,
          { message: 'User not found', success: false, err },
          400,
        );
      }
    }
...
const getByChallenges = async function (req, res) {
  ...
  try {
    record = await ChallengeTracker.paginate(query, options).then(function (
      docs,
      err,
    ) {
      ...
      return ReS(
        res,
        {
          message: 'The Challenge tracking list are',
          success: true,
          challenges: docs,
        },
        200,
    ...
    return ReE(
      res,
      {
        message: 'Error on retrieving challenge tracking list',
        err,
        success: false,
      },
      400,
  ...
  try {
    record = await ChallengeTracker.paginate(query, options).then(function (
      ...
      return ReS(
        res,
        {
          message: 'The Challenger tracking list are',
          success: true,
          challenges: docs,
        },
        200,
  ...
  try {
    record = await ChallengeTracker.paginate(query, options).then(function (
      ...
      return ReS(
        res,
        {
          message: 'The Challenger tracking list of a challenge',
          success: true,
          challenges: docs,
        },
        200,
    ...
    return ReE(
      res,
      {
        message: 'Error on retrieving tracking list of challenge by user',
        err,
        success: false,
      },
      400,
...
const getTracking = async function (req, res) {
  ...
  id = req.params.id[(err, record)] = await to(ChallengeTracker.findById(id));
  ...
  if (err) {
    ...
    if (record) {
      return ReS(
        res,
        { message: 'Trak information is...', Track: record, success: true },
        200,
      );
    } else {
      return ReE(
        res,
        { message: 'Cannot find this record', success: false },
        400,
      );
    }
  }
...
const getLeaders = async function (req, res) {
  ...
  [err, leaders] = await to(
    ChallengeTracker.find({})
      .limit(5)
      .sort({ streakNo: -1, totalkm: -1 })
      .populate([
        { path: 'competitor', select: '_id fullName imageUrl' },
        'challenge',
      ]),
  ...
  if (err) {
    ...
    return ReS(
      res,
      {
        message: 'The Leaderboard details are',
        success: true,
        leaders: leaders,
      },
      200,
...
const getAllWinners = async function (req, res) {
  ...
  [err, leaders] = await to(
    ChallengeTracker.find({ status: 'completed' })
      .limit(10)
      .sort({ prize: -1, streakNo: -1, totalkm: -1 })
      .populate([
        { path: 'competitor', select: '_id fullName imageUrl' },
        'challenge',
      ]),
...
const getChallengeWinner = async function (req, res) {
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'Winners list is ', success: true, winners: winners },
      200,
    );
  }
};
