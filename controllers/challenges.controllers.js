...
const createChallenge = async function (req, res) {
  let body = {
    ...
    totalCompetitors: req.body.totalCompetitors
      ? req.body.totalCompetitors
      : '',
    ...
    competitors: [
      { userId: req.body.createdBy, joinedAt: moment().utc().format() },
    ],
    ...
    if (user) {
      ...
      if (Number(user.gwei) > Number(gwei)) {
        ...
        if (err) {
          return ReE(
            res,
            { message: 'Error on create challenge', success: false, err },
            400,
          );
        } else {
          if (challenge !== null || challenge !== {}) {
            user.gwei = new BigNumber(user.gwei)
              .minus(new BigNumber(gwei))
              .toString();
            ...
            return ReE(
              res,
              {
                message: 'Unable to retrieve challenges please try again.',
                success: false,
              },
              400,
            );
        ...
        return ReE(
          res,
          {
            message: 'You do not have enough gwei to create this challenge',
            success: false,
          },
          400,
        );
...
const getChallenges = async function (req, res) {
  ...
  try {
    challenges = await Challenges.paginate(query, options).then(function (
      docs,
      err,
    ) {
      ...
      return ReS(
        res,
        { message: 'Challenges are', success: true, challenges: docs },
        200,
      );
    });
  } catch (error) {
    return ReE(
      res,
      { message: 'Error on retrieving challenge', err, success: false },
      400,
    );
  }
};
...
//  My Challenges Function

const myChallenges = async function (req, res) {
  let options, query, page, limit, id;
  ...
  try {
    challenges = await Challenges.paginate(query, options).then(function (
      ...
      return ReS(
        res,
        { message: 'My Challenges are', success: true, challenges: docs },
        200,
      );
    });
  } catch (error) {
...
const updateChallenge = async function (req, res) {
  ...
  id = req.query.id[(err, challenge)] = await to(
    Challenges.findByIdAndUpdate(id, { $set: req.body }, { new: true }),
  );
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'Challenge updated', challenge: challenge, success: true },
      200,
    );
  }
};
...
const deleteChallenge = async function (req, res) {
  ...
  id = req.query.id;
  body = req.body[(err, challenges)] = await to(
  ...
  if (err) {
    ...
    return ReS(
      res,
      { message: 'Challenge deleted', challenges: challenges, success: true },
      200,
    );
  }
};
        ...
        if (
          ...
          if (Number(user.gwei) > Number(challenge.wage)) {
            if (isEnded <= 0) {
              if (challenge.competitors.length > challenge.totalCompetitors) {
                return ReE(
                  ...
                  {
                    message:
                      'Total members limit reached please try another challenge.',
                    success: false,
                  },
                  400,
                );
                ...
                if (challenge.competitors.length > 0) {
                  hasJoined = challenge.competitors.filter(
                ...
                if (hasJoined.length < 1) {
                  ...
                  user.gwei = new BigNumber(user.gwei)
                    .minus(new BigNumber(challenge.wage))
                    .toString();
                  ...
                  return ReS(
                    res,
                    {
                      message: 'You have joined this challenge',
                      success: true,
                      challenge: challenge,
                    },
                    200,
                  ...
                  return ReE(
                    res,
                    {
                      message: 'You have already joined in this challenge',
                      success: false,
                    },
                    400,
                  );
              ...
              return ReE(
                res,
                { message: 'Challenge is completed.', success: false },
                400,
              );
            }
          } else {
            return ReE(
              res,
              {
                message: "You don't have enough gwei to join challenge.",
                success: false,
              },
              400,
            );
          ...
          return ReE(
            res,
            { message: 'Invalid challenge.', success: false },
            400,
          );
        }
      }
...
const joinedChallenges = async function (req, res) {
  ...
  if (err) {
    ...
    if (user !== null && user !== {} && user !== undefined) {
      ...
      try {
        Challenges.aggregatePaginate(
          Challenges.aggregate(query),
          options,
          async function (err, results) {
            if (err) {
                ...
                results.docs.map(async (challenge) => {
                  try {
                    await updateStreak(
                      ...
                      userId,
                      body.token,
                      body.distance ? body.distance : false,
                    );
                  } catch (error) {
              ...
              options = {
                ...
                populate: [
                  { path: 'competitor', select: '_id fullName imageUrl' },
                  'challenge',
                ],
              };
              ...
              try {
                record = await ChallengeTracker.paginate(query, options).then(
                  function (docs, err) {
                    ...
                    return ReS(
                      res,
                      {
                        message: "The Challenger's activities are",
                        success: true,
                        challenges: docs,
                      },
                      200,
                ...
                return ReE(
                  res,
                  {
                    message: 'Error on retrieving challenger tracking list',
                    error,
                    success: false,
                  },
                  400,
...
const getChallengeInfo = async function (req, res) {
  ...
  if (err) {
    ...
    [error, leaders] = await to(
      ChallengeTracker.find({ challenge: ObjectId(req.params.id) })
        .sort({ prize: -1, updatedAt: -1, streakNo: -1, totalkm: -1 })
        .populate('competitor'),
    );
    ...
    leaders.map((leader) => {
      if (users.length < 1) {
        ...
        if (
          users.filter((user) => user.competitor._id === leader.competitor._id)
            .length < 1
        ) {
          return users.push(leader);
        }
    ...
    return ReS(
      res,
      {
        message: 'This challenge information is',
        challenge: challenge,
        winners: isEnded ? winners : [],
        leaders: users,
        success: true,
      },
      200,
...
async function updateStreak(challenge, userId, token, km) {
  ...
  [err, activity] = await to(
    ChallengeTracker.findOne({
      competitor: ObjectId(userId),
      challenge: ObjectId(challenge._id.toString()),
    }).sort({ updatedAt: -1 }),
  );
  ...
  if (err) {
    return ReE(
      res,
      {
        message: 'Error on checking challenger is active',
        success: false,
        err,
      },
      400,
    ...
    if (activity && moment().utc().isSame(activity.updatedAt, 'day')) {
      if (
        ...
        moment().utc().diff(challenge.endAt, 'days') < 0
      ) {
        let distance;
        ...
        if (distance) {
          ...
          total_distance =
            typeof token !== 'undefined'
              ? parseFloat(activity.totalkm) + distance
              : distance;
          [err, challengeTracker] = await to(
            ChallengeTracker.findOneAndUpdate(
              {
                competitor: ObjectId(userId),
                challenge: ObjectId(challenge._id.toString()),
                updatedAt: new Date(activity.updatedAt),
              },
              { $set: { totalkm: total_distance } },
              { new: true },
            ),
          );
        ...
        throw Error({
          message: 'Challenge Completed Thank you for participating.',
        });
      }
    } else {
      ...
      if (distance) {
        if (activity) {
          ...
          body.streakNo =
            challenge.totalKm <= distance
              ? activity.streakNo + 1
              : body.streakNo;
          body.status =
            challenge.streakDays === body.streakNo
              ? 'completed'
              : isEnded
              ? 'ended'
              : 'inprogress';
...
const getMyActivity = async function (req, res) {
  ...
  let asset = {
    gold: {
      color: '#FF9BB3',
      image:
        'https://momerlin.s3.amazonaws.com/dev/images/spend_reports/gold.jpeg',
    },
    down: {
      color: '#EB9355',
      image:
        'https://momerlin.s3.amazonaws.com/dev/images/spend_reports/down.jpeg',
    },
  };
  id = req.params.id;

  startDate =
    req.query.startDate !== undefined && req.query.startDate !== null
      ? moment(req.query.startDate).toISOString()
      : null;

  lastDate =
    req.query.lastDate !== undefined && req.query.lastDate !== null
      ? moment(req.query.lastDate).toISOString()
      : null;

  let query =
    startDate !== null && to !== null
      ? {
          competitor: ObjectId(id),
  ...
  [err, activities] = await to(
    ChallengeTracker.find(query).sort({ updatedAt: -1 }).populate('challenge'),
  );
  ...
  if (activities.length > 0) {
    ...
    activities.map((data) => {
      if (data.challenge !== null) {
        if (result.length < 1) {
          ...
          return result.push({
            title: `${data.challenge.totalKm} km ${
              data.challenge.mode
            } ${data.challenge.type.toLowerCase()}`,
            date: moment(data.endAt).toISOString(),
            image:
              data.status === 'completed' ? asset.gold.image : asset.down.image,
            color:
              data.status === 'completed' ? asset.gold.color : asset.down.color,
            amount:
    ...
    return ReS(
      res,
      { message: 'Activity result', success: true, activities: result },
      200,
    );
  } else {
    return ReS(
      res,
      { message: 'Activity result', success: true, activities: [] },
      200,
    );
  }
};
...
async function calculateWinner(id) {
  if (id) {
    let competitors, err;
    [err, competitors] = await to(
      ChallengeTracker.find({
    ...
    if (err) {
      ...
      let prize,
        winners = [];
      prize = parseInt(
        (competitors[0].challenge.prize -
          competitors[0].challenge.prize *
            parseInt(competitors[0].challenge.percentage)) /
          competitors.length,
      );
      if (competitors.length > 0) {
        competitors.map(async (data, index) => {
          ...
          [err, winner] = await to(
            ChallengeTracker.findOneAndUpdate(
              { competitor: ObjectId(data.competitor._id) },
              { $set: { prize: prize } },
              { new: true },
            ),
          );
      ...
      body = { winners: winners }[(err, challenge)] = await to(
        Challenges.findByIdAndUpdate(
          req.query.id,
          { $set: body },
          { new: true },
